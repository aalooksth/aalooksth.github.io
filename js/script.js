document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyThemeMode(mode) {
        ...
        // Day/Night determination based on season
        let isDay = false;
        if (seasonName === 'spring' || seasonName === 'summer') {
            isDay = (hour >= 6 && hour < 19); // 6am to 7pm
        } else {
            isDay = (hour >= 7 && hour < 17); // 7am to 5pm
        }
        ...
        if (mode === 'dark') {
            ...
            // In auto mode, we sync the time-of-day visuals with the actual time, 
            // AND we sync the light/dark theme with the time of day.
            isDark = !isDay;
    ...
    function updateClock() {
        ...
        clockEl.innerHTML = `<span style="font-weight: 500;">${dateStr} ${timeStr}</span>` + 
            (bsStr ? `<span style="color:var(--text-muted); font-size: 0.8em; margin-left: 0.5rem; border-left: 1px solid var(--border-color); padding-left: 0.5rem;">Bi.Sa. ${bsStr}</span>` : "");
